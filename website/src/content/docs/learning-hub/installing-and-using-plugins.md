...
*(v1.0.79+)* You can set `"autoUpdate": true` on a marketplace entry to **automatically update all plugins from that marketplace at session start** — no manual `copilot plugin update` needed:
...
This is useful for internal team marketplaces where you always want the latest versions without extra steps.
