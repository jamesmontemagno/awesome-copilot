...
To automatically update a marketplace's plugins at the start of every session, add `"autoUpdate": true`:
...
With `autoUpdate` enabled, Copilot will fetch the latest plugin versions from this marketplace each time a session starts, so your team always has the most current tooling without needing to run `copilot plugin update` manually.
