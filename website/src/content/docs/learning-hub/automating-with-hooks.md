...
> **Exit code semantics** (v1.0.70+): Exit code **2** is the canonical code for explicitly denying a tool call. Other non-zero codes also deny the call, but exit code 2 is recommended when your hook's purpose is to block a specific tool — it signals a clean, intentional denial rather than an unexpected failure.
