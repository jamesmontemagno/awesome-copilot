...
> **Exit code 2 (v1.0.70+)**: Exiting with code **2** from a `preToolUse` hook specifically **denies the tool call** and signals to the agent that the tool is being blocked by policy — this is the recommended exit code for policy-based denials. Other non-zero exit codes also deny the call, but code 2 is semantically distinct as an intentional policy block rather than a hook script error.
