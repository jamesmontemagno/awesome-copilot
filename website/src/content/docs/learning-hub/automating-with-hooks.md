...
The `preToolUse` hook receives JSON input with details about the tool being called. Your script can inspect this input and exit with the appropriate code:

| Exit Code | Meaning |
|-----------|---------|
| `0` | **Approve** — allow the tool call to proceed |
| `2` | **Deny** — explicitly block the tool call (v1.0.70+) |
| Other non-zero | **Error** — treated as a hook failure; behaviour may vary |

> **Important (v1.0.70+)**: Use exit code `2` to explicitly deny a tool call. Previously, any non-zero exit code was treated as a denial; exit code `2` is now the canonical denial signal for `preToolUse` hooks.
