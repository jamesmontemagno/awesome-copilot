...
Since v1.0.72, you can also install skills through the plugins command — useful when installing from a file, URL, or directory and optionally scoping it to the current project:

```bash
copilot plugins install --skill ./my-skill/                      # install into user skills
copilot plugins install --skill ./my-skill/ --scope project      # install into repository
copilot plugins install --skill https://example.com/my-skill.zip # install from a URL
```

The `--scope project` flag installs the skill into your repository's `.github/skills/` directory so all team members get it automatically.
