...
**Re-enabling a disabled server for one run** *(v1.0.80+)*: If a server is disabled in settings but you need it for a single session, use the `--enable-mcp-server` startup flag to re-enable it without permanently changing your saved setting:

```bash
copilot --enable-mcp-server postgres
```

This is useful when a server is disabled by default for most sessions (to reduce startup time or noise) but occasionally needed for specific tasks.
