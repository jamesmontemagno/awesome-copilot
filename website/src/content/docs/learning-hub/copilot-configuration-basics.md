...
>
> **Note**: `/allow-all auto` requires **experimental mode** to be active. Enable it first with `/experimental on` or by launching Copilot with the `--experimental` flag before using `/allow-all auto`.
