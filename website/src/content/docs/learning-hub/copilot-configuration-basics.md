...
**Model picker grouping** (v1.0.79+): The model picker now groups models into **Recent**, **Recommended**, **New**, and other sections. Use **Shift+Tab** to switch between grouping views, making it easier to discover new models or return to ones you've used before.
...
> **Breaking change (v1.0.79+)**: `/model` is now **session-scoped by default** — changing the model only affects the current session. To set your preferred model for _all future sessions_, use `/config model` instead:
>
> ```
...
> ```
>
> If you were previously using `/model` to set a persistent model preference, switch to `/config model`.
...
**`/worktree new`** (v1.0.79+) starts a **brand-new session** in a fresh worktree, rather than moving the current session:

```
/worktree new                   # new session in a new worktree from HEAD
/worktree new my-feature        # new session in a named worktree
```

Use `/worktree new` when you want to start a completely fresh session on a parallel branch while keeping your current session active and unchanged.

**`worktreeBaseRef` setting** (v1.0.79+): By default, `/worktree`, `/worktree new`, and `--worktree` all start from `HEAD`. You can change this in your config to always start from the remote default branch instead:
...
**Queuing prompts and commands** (v1.0.79+): In local sessions, you can queue prompts, shell commands, and supported slash commands to run in order after the current task finishes. Type your next prompt while the agent is still working and it will be held until the agent is ready — useful for chaining a series of related tasks without waiting between each one.
...
**Combining `--plan` with `--mode autopilot`** (v1.0.79+): You can combine these flags to first create a plan, then automatically implement it without waiting for manual approval:

```bash
copilot --plan --mode autopilot "Add rate limiting to the login endpoint"
```

The CLI presents a plan and immediately proceeds to implementation in autopilot mode — useful for well-defined tasks where you trust the agent to execute after planning but don't want the plan/approval round-trip.
