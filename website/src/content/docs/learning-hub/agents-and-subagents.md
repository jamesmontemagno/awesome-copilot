...
### Sending follow-up messages to running subagents *(v1.0.72+)*

Multi-turn subagents are now **always enabled** in Copilot CLI. This means you can send follow-up messages to a subagent that is currently running, without waiting for it to finish. Use `/tasks` to open the subagent timeline and send steering messages mid-execution. Subagent timelines also identify whether each prompt came from the main agent or from another subagent, making it easier to trace complex orchestration flows.
