...
> **Default depth reduced (v1.0.71+)**: The default maximum sub-agent nesting depth was lowered from 6 to **4**. This prevents runaway recursive delegation in most use cases. Usage-based billing users can raise the limit via the `subagents.maxDepth` setting (up to 128) if their orchestration patterns require deeper nesting.
